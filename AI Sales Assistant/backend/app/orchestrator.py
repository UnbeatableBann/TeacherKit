...
from app.models.domain import ExtractedRequirement, FollowUp, Message, Recommendation
...
async def run_orchestrator(
    conversation_id: str, payload: MessageCreate, db: AsyncSession
) -> OrchestratorResponse:
    # 1. Normalize Input
    try:
    ...
    for msg in normalized_input.messages:
        db_message = Message(conversation_id=conversation_id, role=msg.role, content=msg.content)
        db.add(db_message)
    ...
    generation_result = await generate_follow_up(
        normalized_input=normalized_input, analysis=analysis, rag_context=rag_context_str
    )

    # 5. Persist Everything
    # Persist Requirements
    db_req = await db.execute(
    ...
    # Format objections
    obj_schemas = [
        ObjectionSchema(type="general", text=obj, status="unresolved")
    ...
    for rec in generation_result.recommendations:
        rec_schemas.append(
            RecommendationSchema(name=rec.name, reasoning=rec.reason, sources=rec.sources)
        )
    ...
    return OrchestratorResponse(
        ...
        follow_up_message=generation_result.follow_up_message,
        answered_questions=[],  # Out of scope for this V1
        unanswerable_questions=[],
        lead_score=LeadScoreSchema(score=0, breakdown={"reason": "Not implemented in V1"}),
        next_best_action=NextBestActionSchema(action="Not available", reason="Not implemented in V1"),
        conversation_summary="Not available",
        escalation=EscalationSchema(triggered=False, reason="Not available"),
    )
